package com.orderprocessing.common.response;
...
public class ResponseUtils {
	...
	public static <T> ResponseEntity<PagedResponse<T>> getPagedResponse(String servicePath, Page<T> page) {
		final Sort sortBy = page.getSort();
		final String nextLink = (page.hasNext()) ?
				buildLink(servicePath, page.getNumber() + 1, page.getSize(), sortBy) : null;
	...
	private static String buildLink(String servicePath, int pageNo, int pageSize, Sort sortBy) {
		if (null == sortBy) {
			return String.format(Constants.PAGE_LINK_TEMPLATE, servicePath, pageNo, pageSize);
		}
		return String.format(Constants.PAGE_LINK_SORT_TEMPLATE, servicePath, pageNo, pageSize, getSortParams(sortBy));
	}
